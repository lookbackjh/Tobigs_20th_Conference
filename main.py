...
def main():
    ...
    ## 2. 데이터 전처리
    seq_length=30 # 얼마의 기간을 가지고 다음 기간을 예측할 것인가
    pred_length=5 # 다음 기간을 얼마나 예측할 것인가 ex) 다음 5일에 대한 예측치를 한번에 제공
    ...
    train_s_label=np.zeros(len(train_xs))
    train_t_label=np.ones(len(train_xt))
    ...
    source_trainer=pl.Trainer(max_epochs=1)
    ...
    discriminator_trainer=pl.Trainer(max_epochs=1)
    ...
    test_xt=torch.tensor(test_xt,dtype=torch.float32).to(source_model.device)
    #test_xt=test_xt.unsqueeze(2)
    source_output=source_model.forward(test_xt)
    source_output=source_output.squeeze()
    source_output=source_output.detach().numpy()
    source_output=source_output.reshape(-1,1)
    # evaluate rmse
    ...
    tar_pred=tar_pred.detach().numpy()
    tar_pred=tar_pred.reshape(-1,1)
    # evaluate rmse
    from sklearn.metrics import mean_squared_error
    from math import sqrt
    rmse = sqrt(mean_squared_error(test_yt, tar_pred))
    print('Test RMSE using target only: %.3f' % rmse)
